...
class SaleAdmin(admin.ModelAdmin):
    ...
    @admin.display(ordering='total__sum')
    def calculated_total(self, obj):
        # to handle multiple querysets
        queryset = SaleItem.objects.filter(sale_id=obj.id)\
            .annotate(total=ExpressionWrapper(F('product__price')*F('quantity'), output_field=DecimalField()))
